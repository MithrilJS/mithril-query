...
// Description of Mithril's own internal representation of the DOM.
// Create by m.render()
var mithrilLang = cssauron({
  tag: 'tag',
...
function scan(render) {
  ...
  var parentAttr, childAttr, language;
  if(api.rootEl && api.rootEl.nodeName){
    parentAttr = 'parentNode';
    childAttr = 'childNodes';
    language = domLang;
  } else {
    parentAttr = 'parent';
    childAttr = 'children';
    language = mithrilLang;
  }
  ...
  function select(matchesSelector) {
    return function matches(el, treePath) {
      ...
      if (!el[childAttr] || isString(el[childAttr])) {
        return foundEls;
      }
      el[childAttr].filter(identity).forEach(function(child) {
        // ignore text and number nodes
        if ((typeof child !== 'string') && (typeof child !== 'number')) {
          child[parentAttr] = el;
        }
      });
      return foundEls.concat(matches(el[childAttr], treePath));
    };
  }
  ...
  function contains(value, el) {
    ...
    if (isString(el[childAttr])) {
      return el[childAttr].indexOf(value) >= 0;
    }
    if (isNumber(el)) {
      return el === value;
    }
    if (isNumber(el[childAttr])) {
      return el[childAttr] === value;
    }
    if (isArray(el)) {
    ...
    if (el[childAttr] && el[childAttr].length) {
      return el[childAttr].some(function(child) {
        return contains(value, child);
      });
    }
    return false;
